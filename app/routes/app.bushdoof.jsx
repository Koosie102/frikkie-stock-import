...
export const loader = async ({ request }) => {
  ...
  if (unmatched.length > 0) {
    const vendorProducts = await fetchVendorProducts(admin, VENDOR_NAMES.BUSHDOOF);
...
export const action = async ({ request }) => {
  ...
  if (intent === "updateAllPrices") {
    try {
      const result = await bulkSyncPrices(admin, db, "BUSHDOOF");
  ...
  if (intent === "syncWithShopify") {
    const pushedItems = await db.stagedProduct.findMany({
      where: { source: "BUSHDOOF", status: "PUSHED", shopifyProductId: { not: null } },
...
function ShopifyTagsCard({ vendorTags }) {
  ...
  return (
    <Card>
      <BlockStack gap="300">
        <Text as="h2" variant="headingMd">Existing Shopify tags for Bushdoof</Text>
        <Text as="p" tone="subdued">
          Tags already used on Bushdoof products in your store, most-used
